					...
					<cfloop query="qryAnnouncements">
						...
						<tr class="tableRow#rowClass#">
							...
							<cfif Secure("Approve Messages") OR (qryAnnouncements.sending_user EQ Request.user_id)>
								<a href="#Request.self#?fuseaction=#XFA.edit#&announcement_id=#qryAnnouncements.announcement_id#&success=maybe">Edit</a>
							<cfelse>
								&nbsp;
							</cfif>
							</td>
...
<!--- 
						<cfif attributes.status IS "Saved for Later">
							<td>
								<a href="#Request.self#?fuseaction=#XFA.delete#&announcement_id=#VAL(qryAnnouncements.announcement_id)#" onclick="return confirm('Delete #JsStringFormat(qryAnnouncements.subject)#?');">Delete</a>
							</td>
						</cfif>
--->
						</tr>
