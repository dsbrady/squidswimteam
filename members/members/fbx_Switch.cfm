...
----------------------------------------- --->
	<cfcase value="start,dsp_start,fusebox.defaultfuseaction">
		...
		<cfscript>
			...
			swimBalance = cfcLookup.getMembers(Request.dsn,Request.usersTbl,Request.user_statusTbl,Request.practice_transactionTbl,Request.transaction_typeTbl,Request.squid.user_id).balance;
...
----------------------------------------- --->
	...
	<!--- Displays Buy Swims screen --->
		<cfscript>
			Request.page_title = Request.page_title & "<br />Buy Swims";
			Request.template = "dsp_buySwims.cfm";

			//Get current balance
			cfcLookup = CreateObject("component",Request.lookup_cfc);
			currentBalance = cfcLookup.getMembers(Request.dsn,Request.usersTbl,Request.user_statusTbl,Request.practice_transactionTbl,Request.transaction_typeTbl,Request.squid.user_id).balance;

			XFA.next = "members.dsp_buySwims_confirm";
		</cfscript>
	...
	<!--- Processes swims and sends on to PayPal --->
		<cfscript>
			...
			************************************ */
			...
			Request.paypal_item = "SQUID+Swims+Purchase";
	...
	<!--- Displays Buy Swims results screen (after PayPal) --->
		<cfscript>
			XFA.buySwims = "members.dsp_buySwims";
			Request.page_title = Request.page_title & "<br />Buy Swims Completed";
			lookupCFC = CreateObject("component",Request.lookup_cfc);
