...
<cfsilent>
	...
	<cfif NOT attributes.addNew>
		...
		<cfif (attributes.success IS NOT "No")>
			<cfscript>
				...
				attributes.username = strProfile.username;
	...
	<cfscript>
		editableFields = "";
		viewableFields = "first_name,middle_name,last_name,preferred_name,address,email,birthday,phone,emergency,first_practice,intro_pass,usms,comments,picture,officer,email_preference,posting_preference,calendar_preference,mailingListYN,";

		if (attributes.user_id EQ Request.squid.user_id AND (CompareNoCase(attributes.actionType,"view") NEQ 0))
		{
			editableFields = editableFields & "username,first_name,middle_name,last_name,preferred_name,address,email,birthday,phone,emergency,first_practice,intro_pass,usms,medical,comments,picture,profile_visible,email_preference,posting_preference,calendar_preference,mailingListYN,";
			viewableFields = viewableFields & "username,medical,status,balance,profile_visible,date_expiration,";
		}
		if (Secure("Update Members") AND (CompareNoCase(attributes.actionType,"view") NEQ 0))
		{
			editableFields = editableFields & "username,first_name,middle_name,last_name,preferred_name,address,email,birthday,phone,emergency,first_practice,intro_pass,usms,medical,comments,picture,status,profile_visible,date_expiration,email_preference,posting_preference,calendar_preference,mailingListYN,";
			viewableFields = viewableFields & "username,medical,status,balance,profile_visible,date_expiration,";
		}
	</cfscript>
</cfsilent>
...
<cfif (strProfile.profile_visible) OR (attributes.user_id EQ Request.squid.user_id) OR (Secure("Update Members"))>
	...
	<cfif ListFindNoCase(viewableFields,"address")>
		<tr valign="top">
			...
			<td>
			<cfif ListFindNoCase(editableFields,"address")>
				<input type="text" name="country" size="20" maxlength="50" value="#attributes.country#" tabindex="#variables.tabindex#" />
				<cfset variables.tabindex = variables.tabindex + 1>
			<cfelse>
				#attributes.country#
			</cfif>
	...
	<cfif ListFindNoCase(viewableFields,"email")>
		<tr valign="top">
			...
			<cfif ListFindNoCase(editableFields,"email")>
				<input type="text" name="email" size="20" maxlength="255" value="#attributes.email#" tabindex="#variables.tabindex#" />
				<cfset variables.tabindex = variables.tabindex + 1>
			<cfelse>
				#attributes.email#
			</cfif>
	...
	<cfif ListFindNoCase(viewableFields,"balance")>
		<tr valign="top">
			<td>
...
<cfelse>
	...
	<input type="hidden" name="username_old" value="#strProfile.username#" />
</form>
</cfoutput>
